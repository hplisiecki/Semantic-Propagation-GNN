...
class SPropGNN(nn.Module):
    """
    The main model class for the SPropGNN model for discrete metric prediction, which consists of a sequence of GNN layers followed by
    ...
    """
    def __init__(self, num_node_features, num_node_types, num_relations, num_classes, dropout_prob = 0.3):
        super(SPropGNN, self).__init__()

        # Initialize number of relations and classes
        ...
        self.output_layer = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout_prob),
            nn.Linear(512, num_classes), # The model returns a vector of logits for each class
        )
    ...
    def forward(self, data):
        ...
        self.activations['s_ij_list'] = [layer.last_s_ij.detach().cpu() for layer in self.relational_graph.layers if isinstance(layer, SPropConv)]

        # Concatenate node features with their embeddings
        x_with_embeddings = torch.cat([x, node_type_embedding], dim=-1)

        # Apply attention pooling and store attention weights
        ...
        outputs = self.output_layer(text_embedding)
