...
def get_valence_arousal(words):
    model_dir = r'D:\PycharmProjects\roberta\roberta_base_transformers'
    tokenizer = PreTrainedTokenizerFast(tokenizer_file=os.path.join(model_dir, "tokenizer.json"))
    tokenizer.pad_token = '0'
    metric_names = ['valence', 'arousal', 'dominance', 'origin', 'significance', 'concreteness', 'imageability',
                    'aqcuisition']
    model_name = ["bert"]
    model_initialization = [RobertaModel.from_pretrained(model_dir)]
    model = BertRegression(model_name, model_initialization, metric_names)
    model.load_state_dict(torch.load(r'D:\GitHub\bias_free_modeling\models/polish.pth'), strict=False)
    ...
    return new_columns['valence'], new_columns['arousal']