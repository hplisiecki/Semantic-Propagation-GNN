from english.model import SPropGNN
...
SAVE_DIR = r'D:\GitHub\bias_free_modeling\models\dimensional_english_golden'

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
# device = 'cpu'
model = model.to(device)
criterion = criterion.to(device)
wandb.init(project='bias_free_modeling_english_dimensions', name = 'both_node_features')


train_model(model, optimizer, criterion, train_dataloader, val_dataloader, num_epochs, metric_names, device,
            save_dir = SAVE_DIR, use_wandb = True, scheduler = None)
